    ...
    socket.on("message", async (data) => {
        ...
        switch (validMessage.data.type) {
            ...
            case "chat_message": {
                ...
                if (!socketList?.some((conn) => conn.userId === validMessage.data.userId && conn.socket === socket)) {
                    socket.send(JSON.stringify({
                        type: "error_message",
                ...
                try {
                    const addChat = await client_1.default.chat.create({
                        data: {
                ...
                try {
                    ...
                    switch (drawData.type) {
                        case "create":
                            ...
                            addedDraw = await client_1.default.draw.create({
                                data: {
                                    id: draw.id,
                                    shape: draw.shape,
                        ...
                        case "resize":
                            draw = drawData.modifiedDraw;
                            addedDraw = await client_1.default.draw.update({
                                where: { id: draw.id },
                                data: {
                                    startX: draw.startX,
                                    startY: draw.startY,
                        ...
                        case "erase":
                            ...
                            addedDraw = await client_1.default.draw.delete({
                                where: { id: draw.id },
                            });
                            break;
                    }
