export const renderDraws = (ctx, canvasCurrent, diagrams, activeDraw, selectionBox, activeAction, selectedDraw, toErase, panOffset, scale) => {
    ctx.save();
    ...
    diagrams.forEach((diagram) => {
        ...
        if (toErase?.includes(diagram)) {
            ctx.strokeStyle = ctx.strokeStyle.concat("20");
...
function renderRectangle(ctx, diagram) {
    const cornerRadius = Math.min(40, Math.min(Math.abs(diagram.endX - diagram.startX), Math.abs(diagram.endY - diagram.startY)) * 0.2, Math.min(Math.abs(diagram.endX - diagram.startX), Math.abs(diagram.endY - diagram.startY)) / 2);
    ctx.beginPath();
...
function renderDiamond(ctx, diagram) {
...
function renderCircle(ctx, diagram) {
...
function renderLine(ctx, diagram) {
    ctx.beginPath();
...
function renderArrow(ctx, diagram) {
    const p0 = { x: diagram.startX, y: diagram.startY };
    const p1 = diagram.points[0];
    const p2 = { x: diagram.endX, y: diagram.endY };
    const controlPointX = 2 * p1.x - 0.5 * p0.x - 0.5 * p2.x;
    ...
    const lineLength = Math.sqrt(Math.pow(p2.x - p0.x, 2) + Math.pow(p2.y - p0.y, 2));
    const headLength = Math.min(lineLength * 0.2, 20) + (diagram.lineWidth ?? 1) * 2;
    ctx.save();
    ctx.beginPath();
    ctx.moveTo(p2.x, p2.y);
    ctx.lineTo(p2.x - headLength * Math.cos(angle - Math.PI / 10), p2.y - headLength * Math.sin(angle - Math.PI / 10));
    ctx.moveTo(p2.x, p2.y);
    ctx.lineTo(p2.x - headLength * Math.cos(angle + Math.PI / 10), p2.y - headLength * Math.sin(angle + Math.PI / 10));
    ctx.stroke();
    ctx.restore();
}
function renderFreeHand(ctx, diagram) {
    ...
    ctx.lineTo(diagram.points[diagram.points.length - 1].x, diagram.points[diagram.points.length - 1].y);
    ctx.stroke();
}
function renderText(ctx, diagram) {
...
function renderCursor(ctx, diagram) {
    ctx.font = `${diagram.fontSize}px ${diagram.font}`;
...
function renderSelectionBox(ctx, selectionBox) {
    ...
    ctx.beginPath();
    ctx.strokeRect(selectionBox.startX, selectionBox.startY, selectionBox.endX - selectionBox.startX, selectionBox.endY - selectionBox.startY);
    ...
    if (selectionBox.text === "text") {
        ...
        if (selectionBox.points.length === 3) {
            ...
            ctx.arc(selectionBox.points[0].x, selectionBox.points[0].y, 4, 0, 2 * Math.PI);
            ctx.moveTo(selectionBox.points[1].x, selectionBox.points[1].y);
            ctx.arc(selectionBox.points[1].x, selectionBox.points[1].y, 4, 0, 2 * Math.PI);
            ctx.moveTo(selectionBox.points[2].x, selectionBox.points[2].y);
            ctx.arc(selectionBox.points[2].x, selectionBox.points[2].y, 4, 0, 2 * Math.PI);
            ctx.fill();
            ctx.fillStyle = "#5588ff70";
            ctx.moveTo(selectionBox.points[0].x, selectionBox.points[0].y);
            ctx.arc(selectionBox.points[0].x, selectionBox.points[0].y, 8, 0, 2 * Math.PI);
            ctx.moveTo(selectionBox.points[1].x, selectionBox.points[1].y);
            ctx.arc(selectionBox.points[1].x, selectionBox.points[1].y, 8, 0, 2 * Math.PI);
            ctx.moveTo(selectionBox.points[2].x, selectionBox.points[2].y);
            ctx.arc(selectionBox.points[2].x, selectionBox.points[2].y, 8, 0, 2 * Math.PI);
            ctx.fill();
        }
    }
    ctx.closePath();
