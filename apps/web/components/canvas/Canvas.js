...
import { moveDraw, resizeDraw, handleShapeSelectionBox, calculateFarthestPoints, } from "@/lib/canvas/updateFunctions";
import { getDrawAtPosition, hoverOverSelectionBox, } from "@/lib/canvas/selectFunctions";
import { Button } from "@workspace/ui/components/button";
import { useEffect, useRef, useState } from "react";
import { BsFonts } from "react-icons/bs";
import { PiArrowRight, PiChatsCircle, PiCircle, PiCircleFill, PiCursor, PiCursorFill, PiDiamond, PiDiamondFill, PiEraser, PiEraserFill, PiLineVertical, PiLineVerticalLight, PiMinus, PiPencil, PiPencilFill, PiPlus, PiSquare, PiSquareFill, } from "react-icons/pi";
import { LiaHandPaper, LiaHandRock } from "react-icons/lia";
import { performAction, performRedo, performUndo, pushToUndoRedoArray, } from "@/lib/canvas/actionRelatedFunctions";
...
const Canvas = ({ roomId, token }) => {
    const [showChatBar, setShowChatBar] = useState(false);
    const canvasRef = useRef(null);
    const [isClient, setIsClient] = useState(false);
    const { socket, isLoading, isError } = useWebSocket(`${process.env.NEXT_PUBLIC_WS_URL}?token=${token}`);
    ...
    const lastSrNoRef = useRef(0);
    const chatMessageInputRef = useRef(null);
    const unreadMessagesRef = useRef(false);
    const [isLoadingMoreMessages, setIsLoadingMoreMessages] = useState(false);
    const activeDraw = useRef(null);
    const selectedDraw = useRef(null);
    const originalDrawState = useRef(null);
    const modifiedDrawState = useRef(null);
    const movingOffset = useRef({ x: 0, y: 0 });
    const panStartPoint = useRef({ x: 0, y: 0 });
    const scale = useRef(1);
    const intialPointsForFreeHandMove = useRef(null);
    const resizingInfo = useRef(null);
    const farthestPointsInfoForLineAndArror = useRef(null);
    const shapeSelectionBox = useRef(null);
    ...
    const undoRedoArrayRef = useRef([]);
    const editCounterRef = useRef(0);
    const undoRedoIndexRef = useRef(-1);
    const toErase = useRef([]);
    ...
    const handleLoadMoreMessages = async () => {
        if (!lastSrNoRef.current || isLoadingMoreMessages)
            return [];
    ...
    const activeStrokeStyleRef = useRef(activeStrokeStyle);
    ...
    useEffect(() => {
        ...
        if (selectedDraw.current) {
            ...
            if (selectedDraw.current.fontSize === "20" ||
    ...
    function executeUndo() {
        ...
        console.log("undo");
        const changes = performUndo(undoRedoArrayRef.current, undoRedoIndexRef.current, diagrams.current, socket, user.id, roomId);
    ...
    function executeRedo() {
        ...
        console.log("redo");
        const changes = performRedo(undoRedoArrayRef.current, undoRedoIndexRef.current, diagrams.current, socket, user.id, roomId);
        if (shapeSelectionBox.current && canvasRef.current) {
            shapeSelectionBox.current = null;
        }
        diagrams.current = changes.diagrams;
    ...
    useEffect(() => {
        ...
        const handleShortcutsClose = (event) => {
            if (activeDraw.current?.shape !== "text" &&
                selectedDraw.current?.shape !== "text") {
                if (event.key === "Shift") {
                    setActiveAction("select");
    ...
    useEffect(() => {
        ...
        const renderInterval = setInterval(() => {
            renderDraws(ctx, canvasCurrent, diagrams.current, activeDraw.current, shapeSelectionBox.current, activeActionRef.current, selectedDraw.current, toErase.current, panOffset.current, scale.current);
        }, 15);
        ...
        const handleMouseDown = (event) => {
            ...
            if (activeActionRef.current === "select") {
                ...
                if (draw && !hoveredSelectionBox?.position.includes("point")) {
                    ...
                    farthestPointsInfoForLineAndArror.current = calculateFarthestPoints(selectedDraw.current);
                    intialPointsForFreeHandMove.current = {
                        initialPoint: {
                            x: offsetX,
                            y: offsetY,
                    ...
                    originalDrawState.current = JSON.parse(JSON.stringify(selectedDraw.current));
                }
                else {
                    setActiveAction("select");
            ...
            if (activeActionRef.current === "edit") {
                if (selectedDraw.current && selectedDraw.current.shape === "text") {
                    ...
                    if (originalDrawState.current && selectedDraw.current && socket) {
                        ...
                        const { undoRedoArray, undoRedoIndex } = pushToUndoRedoArray(action, undoRedoArrayRef.current, undoRedoIndexRef.current, socket, user.id, roomId);
            ...
            if (activeActionRef.current === "draw") {
                ...
                activeDraw.current = {
                    ...
                    points: isDrawing || isLineOrArrow
        ...
        const handleMouseMove = (event) => {
            ...
            const { offsetX, offsetY } = getMousePosition(event);
            if (activeActionRef.current === "select") {
                const hoveredDraw = getDrawAtPosition(offsetX, offsetY, diagrams.current, ctx);
                const hoveredSelectionBox = hoverOverSelectionBox(shapeSelectionBox.current, offsetX, offsetY);
            ...
            if (activeActionRef.current === "resize") {
                ...
                const draw = resizeDraw(resizingInfo.current, offsetX, offsetY, selectedDraw.current, diagrams.current, farthestPointsInfoForLineAndArror.current, intialPointsForFreeHandMove.current);
                if (!draw)
                    return;
            ...
            if (activeActionRef.current === "move") {
                ...
                const draw = moveDraw(offsetX, offsetY, movingOffset.current.x, movingOffset.current.y, selectedDraw.current, diagrams.current, intialPointsForFreeHandMove.current);
                modifiedDrawState.current = JSON.parse(JSON.stringify(draw));
                if (!draw)
                    return;
                shapeSelectionBox.current = handleShapeSelectionBox(draw, ctx);
            ...
            if (activeActionRef.current === "draw") {
                ...
                if (activeDraw.current.shape === "freeHand") {
                    ...
                    if (activeDraw.current.shape === "line" ||
            ...
            if (activeActionRef.current === "erase" && isErasing.current) {
                const hoveredOver = getDrawAtPosition(offsetX, offsetY, diagrams.current, ctx);
                if (hoveredOver) {
        ...
        const handleMouseUp = (event) => {
            ...
            if (activeActionRef.current === "erase" && isErasing.current) {
                diagrams.current = diagrams.current.filter((draw) => !toErase.current.includes(draw));
            ...
            if (activeActionRef.current === "resize") {
                if (!selectedDraw.current)
                    return;
                if (selectedDraw.current.shape === "rectangle" ||
            ...
            if (activeActionRef.current === "draw") {
                ...
                if (activeDraw.current.shape !== "freeHand") {
                    ...
                    if (activeDraw.current.shape === "rectangle" ||
                    ...
                    else if (activeDraw.current.shape === "line" ||
                        activeDraw.current.shape === "arrow") {
                        activeDraw.current.points = [
                            {
                                x: (activeDraw.current.startX + activeDraw.current.endX) / 2,
        ...
        const handleKeyDown = (event) => {
            ...
            if (activeActionRef.current === "edit") {
                if (selectedDraw.current?.shape === "text") {
                    shapeSelectionBox.current = handleShapeSelectionBox(selectedDraw.current, ctx);
                    return;
                }
                shapeSelectionBox.current = null;
            }
    ...
    const changeActiveFillStyle = (color) => {
        setActiveFillStyle(color);
        if (selectedDraw.current) {
            selectedDraw.current.fillStyle = color;
            if (originalDrawState.current?.fillStyle !==
    ...
    const changeActiveStrokeStyle = (color) => {
        setActiveStrokeStyle(color);
        if (selectedDraw.current) {
            selectedDraw.current.strokeStyle = color;
            if (originalDrawState.current?.strokeStyle !==
    ...
    const changeActiveLineWidth = (width) => {
        setActiveLineWidth(width);
        if (selectedDraw.current) {
            selectedDraw.current.lineWidth = width;
            if (originalDrawState.current?.lineWidth !==
    ...
    const changeActiveFontSize = (size) => {
        setActiveFontSize(size.toString());
        if (selectedDraw.current) {
            selectedDraw.current.fontSize = size.toString();
            if (originalDrawState.current?.fontSize !== selectedDraw.current.fontSize &&
                socket) {
                const action = {
                    type: "edit",
                    originalDraw: JSON.parse(JSON.stringify(originalDrawState.current)),
    ...
    if (isError) {
        return (<div className="h-screen w-screen relative flex items-center justify-center bg-neutral-900">
      ...
      <div className="h-screen w-screen relative">
        <div className="fixed z-2 w-fit h-fit bg-neutral-900 rounded-md left-3 top-3">
          <div className="bg-green-400/25 z-1 rounded-lg px-1.5 py-1 flex gap-1.5 items-center">
            <Button size="icon" className={`bg-transparent relative p-2 hover:bg-green-600/20 cursor-pointer`} onClick={() => {
            closeSocket();
            redirect("/home");
        }}>
        ...
        <div className="fixed z-2 w-fit h-fit bg-black rounded-lg right-3 top-3">
          <div className="bg-green-400/25 z-1 rounded-lg px-1.5 py-1 flex gap-1.5 items-center">
            <Button size="icon" className={`bg-transparent relative p-2 hover:bg-green-600/20 cursor-pointer ${showChatBar ? "bg-green-600/40" : ""} ${unreadMessagesRef.current ? "bg-green-600/5" : ""}`} onClick={() => {
            setShowChatBar(!showChatBar);
            unreadMessagesRef.current = false;
        }}>
              <PiChatsCircle className="text-white" size="18"/>
              {unreadMessagesRef.current && (<div className="absolute top-0 right-0 w-2 h-2 bg-green-500 rounded-full"/>)}
            </Button>
          </div>
        </div>

        {showChatBar && (<ChatBar closeChat={() => setShowChatBar(false)} messages={chatMessages} user={user} onSendMessage={handleSendMessage} onLoadMoreMessages={handleLoadMoreMessages} isLoadingMore={isLoadingMoreMessages} chatMessageInputRef={chatMessageInputRef}/>)}

        <div className="fixed z-2 w-fit h-fit bg-black rounded-lg left-1/2 top-3 transform -translate-x-1/2">
          <div className="bg-green-400/25 z-1 rounded-lg px-1.5 py-1 flex gap-1.5 items-center">
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "select" || activeAction === "move" || activeAction === "resize" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("select");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
                shapeSelectionBox.current = null;
              ...
              {activeAction === "select" ||
            activeAction === "move" ||
            activeAction === "resize" ? (<PiCursorFill className="text-white" size="18"/>) : (<PiCursor className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                1
              </p>
            </Button>
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "rectangle" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("rectangle");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
              ...
              {activeAction === "draw" && activeShape === "rectangle" ? (<PiSquareFill className="text-white" size="18"/>) : (<PiSquare className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                2
              </p>
            </Button>
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "diamond" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("diamond");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
              ...
              {activeAction === "draw" && activeShape === "diamond" ? (<PiDiamondFill className="text-white" size="18"/>) : (<PiDiamond className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                3
              </p>
            </Button>
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "circle" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("circle");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
              ...
              {activeAction === "draw" && activeShape === "circle" ? (<PiCircleFill className="text-white" size="18"/>) : (<PiCircle className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                4
              </p>
            </Button>
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "line" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("line");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
            ...
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "arrow" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("arrow");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
            ...
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "draw" && activeShape === "freeHand" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("freeHand");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
              ...
              {activeAction === "draw" && activeShape === "freeHand" ? (<PiPencilFill className="text-white" size="18"/>) : (<PiPencil className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                7
              </p>
            </Button>
            <Button size="icon" className={`bg-transparent relative p-2 ${(activeAction === "draw" && activeShape === "text") || activeAction === "edit" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("draw");
            setActiveShape("text");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
            ...
            <Button size="icon" className={`bg-transparent relative p-2 ${activeAction === "erase" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("erase");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
                shapeSelectionBox.current = null;
              ...
              {activeAction === "erase" ? (<PiEraserFill className="text-white" size="18"/>) : (<PiEraser className="text-white" size="18"/>)}
              <p className="text-white font-mono absolute text-[8px] right-1 bottom-1">
                9
              </p>
            </Button>
            <PiLineVerticalLight size="20"/>
            <Button size="icon" className={`bg-transparent -ml-1 relative p-2 ${activeAction === "pan" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("pan");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
                shapeSelectionBox.current = null;
              ...
              {activeAction === "pan" && isDragging ? (<LiaHandRock className="text-white"/>) : (<LiaHandPaper className="text-white"/>)}
            </Button>
            <Button size="icon" className={`bg-transparent -ml-0.5 relative p-2 ${activeAction === "zoom" ? "bg-green-600 hover:bg-green-600" : "hover:bg-green-600/20"} cursor-pointer`} onClick={() => {
            setActiveAction("zoom");
            if (activeDraw.current?.shape === "text") {
                activeDraw.current = null;
                shapeSelectionBox.current = null;
        ...
        {activeAction === "draw" ||
            (activeAction === "select" && selectedShape !== null) ? (activeShape === "text" || selectedShape === "text" ? (<div className="fixed px-3 py-2 z-2 w-fit h-fit border border-neutral-600 left-3 top-1/2 transform -translate-y-1/2 bg-black rounded-md">
              <div className="space-y-2 items-center rounded-md text-white">
                <div className="text-sm">
                  <h3>Color</h3>
                  <div className="flex items-center gap-2">
                    <Button size="sm" className="bg-[#eeeeee] hover:bg-[#eeeeee] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveStrokeStyle("#eeeeee");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#FFD586] hover:bg-[#FFD586] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveStrokeStyle("#FFD586");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#FF9898] hover:bg-[#FF9898] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveStrokeStyle("#FF9898");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#B9D4AA] hover:bg-[#B9D4AA] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveStrokeStyle("#B9D4AA");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#8DD8FF] hover:bg-[#8DD8FF] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveStrokeStyle("#8DD8FF");
            }}>
                      ..
                    </Button>
                    <PiLineVerticalLight size="20"/>
                    <Button size="icon" className="relative cursor-default -mr-1" style={{ backgroundColor: activeStrokeStyle }}></Button>
                  </div>
                </div>
                <div className="text-sm">
                  <h3>Font</h3>
                  <div className="flex items-center gap-2">
                    <Button size="sm" className={`relative cursor-pointer text-white font-[Arial] -mr-1 ${activeFont === "Arial" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFont("Arial")}>
                      Abc
                    </Button>
                    <Button size="sm" className={`relative cursor-pointer text-white font-[Verdana] -mr-1 ${activeFont === "Verdana" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFont("Verdana")}>
                      Abc
                    </Button>
                    <Button size="sm" className={`relative cursor-pointer text-white font-[ComicSansMS] -mr-1 ${activeFont === "Comic Sans MS" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFont("Comic Sans MS")}>
                      Abc
                    </Button>
                  </div>
                </div>
                <div className="text-sm">
                  <h3>Font Size</h3>
                  <div className="flex items-center gap-2">
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeFontSize === "20" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFontSize(20)}>
                      S
                    </Button>
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeFontSize === "40" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFontSize(40)}>
                      M
                    </Button>
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeFontSize === "60" ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => changeActiveFontSize(60)}>
                      L
                    </Button>
                  </div>
                </div>
              </div>
            </div>) : activeShape === "freeHand" ||
            activeShape === "arrow" ||
            activeShape === "line" ||
            selectedShape === "freeHand" ||
            selectedShape === "arrow" ||
            selectedShape === "line" ? (<div className="fixed px-3 py-2 z-2 w-fit h-fit border border-neutral-600 left-3 top-1/2 transform -translate-y-1/2 bg-black rounded-md">
              <div className="space-y-2 items-center rounded-md">
                ...
                <div className="text-sm">
                  <h3>Stroke Width</h3>
                  <div className="flex items-center gap-2">
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeLineWidth === 2 ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => {
                changeActiveLineWidth(2);
                    ...
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeLineWidth === 3 ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => {
                changeActiveLineWidth(3);
                    ...
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeLineWidth === 4 ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => {
            ...
            </div>) : (<div className="fixed px-3 py-2 z-2 w-fit h-fit border border-neutral-600 left-3 top-1/2 transform -translate-y-1/2 bg-black rounded-md">
              <div className="space-y-2 items-center rounded-md">
                <div className="text-sm">
                  <h3>Stroke</h3>
                  <div className="flex items-center gap-2">
                ...
                <div className="text-sm">
                  <h3>Background</h3>
                  <div className="flex items-center gap-2">
                    <Button size="sm" className="relative cursor-pointer -mr-1 text-transparent hover:bg-transparent bg-transparent border border-gray-400/20" onClick={() => {
                changeActiveFillStyle("#eeeeee00");
            }}>
                      .
                    </Button>
                    <Button size="sm" className="bg-[#FFD58660] hover:bg-[#FFD58660] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveFillStyle("#FFD58660");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#FF989860] hover:bg-[#FF989860] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveFillStyle("#FF989860");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#B9D4AA60] hover:bg-[#B9D4AA60] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveFillStyle("#B9D4AA60");
            }}>
                      ..
                    </Button>
                    <Button size="sm" className="bg-[#8DD8FF60] hover:bg-[#8DD8FF60] relative cursor-pointer -mr-1 text-transparent" onClick={() => {
                changeActiveFillStyle("#8DD8FF60");
            }}>
                      ..
                    </Button>
                    <PiLineVerticalLight size="20"/>
                    <Button size="icon" className="relative cursor-default -mr-1 border" style={{ backgroundColor: activeFillStyle }}></Button>
                  </div>
                </div>
                <div className="text-sm">
                  <h3>Stroke Width</h3>
                ...
                changeActiveLineWidth(3);
                    ...
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeLineWidth === 6 ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => {
                changeActiveLineWidth(6);
                    ...
                    <Button size="sm" className={`relative cursor-pointer text-white -mr-1 ${activeLineWidth === 9 ? "bg-green-600/40 hover:bg-green-600/40" : "bg-neutral-900 hover:bg-neutral-800"}`} onClick={() => {
        ...
        <div className="fixed flex gap-2 z-2 w-fit h-fit left-3 bottom-3 bg-neutral-900 rounded-md">
          <div className="bg-neutral-900 rounded-md">
            <div className="bg-green-400/25 p-1 flex items-center rounded-md">
              <Button size="icon" className="bg-transparent relative cursor-pointer -mr-1 hover:bg-green-600/40" onClick={() => zoomToPoint(scale.current - 0.1)}>
                <PiMinus className="text-white" size="18"/>
              </Button>
              <PiLineVerticalLight size="20"/>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" className="bg-transparent relative cursor-pointer px-1 py-2 text-white font-mono text-sm h-auto hover:bg-green-600/40" onClick={() => zoomToPoint(1)}>
              ...
              <PiLineVerticalLight size="20"/>
              <Button size="icon" className="bg-transparent relative cursor-pointer -ml-1 hover:bg-green-600/40" onClick={() => zoomToPoint(scale.current + 0.1)}>
          ...
          <div className="bg-neutral-900 rounded-md">
            <div className="bg-green-400/25 p-1 flex gap-2 items-center rounded-md">
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" className="relative cursor-pointer border-r border-green-900 -mr-1 rounded-r-none bg-green-600/40 hover:bg-green-600/60" onClick={executeUndo} disabled={!canUndo}>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button size="icon" className="relative cursor-pointer border-l border-green-900 -ml-1 rounded-l-none bg-green-600/40 hover:bg-green-600/60" onClick={executeRedo} disabled={!canRedo}>
        ...
        {isClient ? (<canvas tabIndex={0} ref={canvasRef} className="bg-neutral-900 absolute top-0 left-0 z-1" width={window.innerWidth} height={window.innerHeight}></canvas>) : (<canvas tabIndex={0} ref={canvasRef} className="bg-neutral-900"></canvas>)}
      </div>
    </TooltipProvider>);
};
