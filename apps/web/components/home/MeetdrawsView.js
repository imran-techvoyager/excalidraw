...
const MeetdrawsView = () => {
    const meetdrawsViewDivRef = useRef(null);
    ...
    const filteredRooms = rooms.filter((room) => room.title.toLowerCase().includes(search.toLowerCase()));
    useEffect(() => {
        if (meetdrawsViewDivRef.current?.scrollHeight &&
            meetdrawsViewDivRef.current?.clientHeight &&
            meetdrawsViewDivRef.current.scrollHeight >
                meetdrawsViewDivRef.current.clientHeight) {
            if (meetdrawsViewDivRef.current?.scrollTop &&
    ...
    return (<>
      <div className="flex w-full items-center justify-between">
        ...
        <div className="relative w-fit flex items-center rounded-lg">
          <Input className="max-w-72 min-w-72 relative focus-visible:border-green-600/50 focus-visible:ring-green-600/20" placeholder="Type to search" value={search} onChange={(e) => setSearch(e.target.value)}></Input>
          <div className="bg-transparent absolute right-0 rounded-lg py-2 pr-3">
            <BiSearch className=""/>
          </div>
        </div>
      </div>
      {filteredRooms.length === 0 && (<div className="flex-1 flex justify-center gap-2 text-neutral-400">
      ...
      <div ref={meetdrawsViewDivRef} className="grid grid-cols-3 auto-rows-min gap-4 flex-1 min-h-0 overflow-y-auto py-1 [&::-webkit-scrollbar]:hidden">
        {filteredRooms.map((room) => (<MeetdrawCard key={room.id} room={room}/>))}
