...
const MeetdrawCard = ({ room }) => {
        ...
        if (canvasRef.current && room.Draw) {
            ...
            if (ctx) {
                ...
                if (drawingWidth > 0 && drawingHeight > 0) {
                    scale = Math.min(canvasWidth / drawingWidth, canvasHeight / drawingHeight);
                ...
                renderDraws(ctx, canvas, draws, null, // activeDraw
                null, // selectionBox
                "draw", // activeAction
                null, // selectedDraw
                [], // toErase
    ...
    return (<Card onClick={() => redirect(`/canvas/${room.id}`)} className="block py-4 row-span-1 overflow-clip border cursor-pointer backdrop-blur-md bg-black/25 hover:-translate-y-[3px] transition-all duration-200 h-fit min-h-0">
      ...
      <CardContent className="w-full h-full mt-4">
        <canvas ref={canvasRef} className="w-full h-full bg-neutral-900/70 rounded-md" style={{ minHeight: "150px" }}></canvas>
      </CardContent>
    </Card>);
};
