...
const ChatRoom = ({ jwtCookie }) => {
    ...
    const [lastSrNo, setLastSrNo] = useState();
    ...
    const { socket, isLoading, isError } = useWebSocket(`${process.env.NEXT_PUBLIC_WS_URL}?token=${jwtCookie.value}`);
    useEffect(() => {
    ...
    return (<div className="flex flex-col gap-2 h-full">
      <div className="flex items-center justify-between px-1">
        ...
        <div className="flex items-center gap-4">
          <Tooltip>
            ...
            <TooltipContent side="bottom" className="text-sm border text-neutral-700">
              <div>
                <h3>Admin: {activeRoom.admin.username}</h3>
                <div className="flex gap-2">
                  <h4>Join Code: {activeRoom.joinCode}</h4>
                  <button className="cursor-pointer" onClick={handleCopyJoinCode}>
                    <BiCopy />
                  </button>
                </div>
              </div>
          ...
          <Link href={`/canvas/${activeRoom.id}`} className="flex items-center gap-1 py-0.5 px-3 bg-neutral-900/60 border border-neutral-800 text-neutral-400 hover:text-neutral-300 hover:bg-neutral-800/50 rounded-sm">
            Canvas
            <GoArrowUpRight />
          </Link>

          <button onClick={() => dispatch(setHomeView("meetdraws"))} className="cursor-pointer text-neutral-500">
            <RxCross1 className="w-4 h-4"/>
          </button>
        </div>
      </div>
      <div className="flex flex-col gap-2 p-2 flex-1 min-h-0 border backdrop-blur-md bg-black/30 rounded-lg overflow-hidden">
        <div ref={chatDivRef} className="flex-1 overflow-y-auto scroll-smooth [&::-webkit-scrollbar]:hidden">
          {isLoadingMore && (<div className="flex justify-center items-center gap-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            messages.map((message, index) => {
                ...
                return (<div key={`${index}`} className={`${message.userId === userState.id
                        ? "self-end rounded-tr-none bg-green-600/15"
                        : "rounded-tl-none bg-neutral-800/50"} w-fit py-2 px-4 rounded-xl max-w-3/4 backdrop-blur-sm`}>
          ...
          {showArrow === "down" && (<button onClick={() => {
              ...
              {showBadge && (<div className="bg-green-500 w-[5px] h-[5px] absolute rounded-full z-1 top-0 right-0"/>)}
            </button>)}
          {showArrow === "up" && (<button onClick={() => {
                chatDivRef.current.scrollTo({
                    top: 0,
                    behavior: "smooth",
                });
      ...
      <form action={handleSendMessage} className="w-full flex items-center gap-2">
        <textarea ref={inputRef} placeholder="Type your message" className="w-full border rounded-md p-2 [&::-webkit-scrollbar]:hidden max-h-[45px] resize-none"/>
        <Button type="submit" className="cursor-pointer rounded-md bg-neutral-500 hover:bg-neutral-400">
          <IoSend className=""/>
        </Button>
      </form>
    </div>);
