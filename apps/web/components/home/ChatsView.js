...
const ChatsView = () => {
    ...
    return (<div className="flex-1 min-h-0">
      <Card className="h-full flex flex-col backdrop-blur-md bg-black/30">
        ...
        <CardContent ref={chatsViewDivRef} className="flex-1 overflow-y-auto min-h-0 flex flex-col gap-2 [&::-webkit-scrollbar]:hidden py-1">
          {rooms.map((room) => (<ChatCard key={room.id} room={room}/>))}
          {rooms.length === 0 && (<div className="flex-1 flex justify-center gap-2 text-neutral-400">
